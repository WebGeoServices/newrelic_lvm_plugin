...
class NrLvmTests(unittest.TestCase):
    ...
    def test_set_datas_from_lvs_command(self, mock_check_output):
        ...
        datas = set_datas()
        assert_that(datas[0]['components'][0]["metrics"]).contains('Component/lvm/usage/Data/Used[percent]')
        assert_that(datas[0]['components'][0]["metrics"]).contains('Component/lvm/usage/Metadata/Used[percent]')
        assert_that(datas[0]['components'][0]["metrics"]['Component/lvm/usage/Data/Used[percent]']).equals(float(12))
        assert_that(datas[0]['components'][0]["metrics"]['Component/lvm/usage/Metadata/Used[percent]']).equals(float(2))
